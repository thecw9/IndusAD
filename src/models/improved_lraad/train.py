...
class Trainer(BaseTrainer):
    def __init__(
        ...
        print(f"  Number of VAE Epochs: {self.num_vae}")
        ...
        print(f"  Negative Beta: {self.beta_neg}")
        ...
        self.optimizer_d = optim.Adam(self.model.decoder.parameters(), lr=lr_d)
        ...
        self.d_scheduler = optim.lr_scheduler.MultiStepLR(
            self.optimizer_d, milestones=(350,), gamma=0.1
        )
    ...
    def train_one_epoch(self):
        ...
        for real, _ in pbar:
            # --------------train------------
            if self.current_epoch < self.num_vae:
                ...
                pbar.set_description_str("Epoch {}".format(self.current_epoch))
                pbar.set_postfix(
                ...
                b_size = real.size(0)
                noise_batch = torch.randn(size=(b_size, self.z_dim)).to(self.device)
                ...
                lossE_kl_rec = kl_divergence(rec_mu, rec_logvar, reduction="none")
                lossE_kl_fake = kl_divergence(fake_mu, fake_logvar, reduction="none")

                lossE_rec_real = reconstruction_loss(
                    real, rec, loss_type=self.recon_loss_type, reduction="mean"
                )
                lossE_rec_rec = reconstruction_loss(
                    rec, rec_rec, loss_type=self.recon_loss_type, reduction="none"
                )
                lossE_rec_fake = reconstruction_loss(
                ...
                expelbo_rec = (
                    (
                        -2
                        * self.scale
                        * (self.beta_rec * lossE_rec_rec + self.beta_neg * lossE_kl_rec)
                    )
                    .exp()
                    .mean()
                )
                expelbo_fake = (
                    (
                        ...
                        * self.scale
                        * (
                            self.beta_rec * lossE_rec_fake
                            + self.beta_neg * lossE_kl_fake
                        )
                    )
                    .exp()
                    .mean()
                ...
                lossE_real = self.scale * (
                    self.beta_rec * lossE_rec_real + self.beta_kl * lossE_kl_real
                )
                ...
                lossG_rec_rec = reconstruction_loss(
                    rec.detach(),
                    rec_rec,
                    loss_type=self.recon_loss_type,
                    reduction="mean",
                )
                lossG_rec_fake = reconstruction_loss(
                    fake.detach(),
                    rec_fake,
                    loss_type=self.recon_loss_type,
                    reduction="mean",
                ...
                lossG_kl_fake = kl_divergence(fake_mu, fake_logvar, reduction="mean")

                lossG = self.scale * (
                    lossG_rec_real * self.beta_rec
                    + (lossG_kl_rec + lossG_kl_fake) * 0.5 * self.beta_kl
                    + self.gamma_r
                    * 0.5
                    * self.beta_rec
                    * (lossG_rec_rec + lossG_rec_fake)
                )
                ...
                pbar.set_postfix(
                    ...
                    kl_fake=lossE_kl_fake.mean().data.cpu().item(),
                    expelbo_fake=expelbo_fake.cpu().item(),
                )
    ...
    def evaluate(self, save_root="results"):
        ...
        save_root = Path(save_root) / f"improved_lraad/{self.dataset}/"
    ...
    def train(self):
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            self.save_checkpoint(
                f"checkpoints/improved_lraad/{self.dataset}/epoch_{self.current_epoch}.pth"
            )
            self.evaluate()
            self.visualize()
